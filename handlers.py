...
async def process_presentation(message: Message, state: FSMContext):
    ...
    prompt = (
        f"Сделай план презентации на тему: {topic}. "
        f"Используй markdown: заголовки слайдов через ###, пункты через -. "
        f"Только структура, 5-7 слайдов, без вступлений."
    )
    if lang == "kz":
        prompt = (
            f"Келесі тақырыпқа презентация жоспарын жаса: {topic}. "
            f"Markdown: слайд тақырыптары ###, пункттер -. "
            f"Тек құрылым, 5-7 слайд, кіріспесіз."
        )

    await message.answer("⏳ Генерирую презентацию с картинками... Подожди ~20 секунд.")
